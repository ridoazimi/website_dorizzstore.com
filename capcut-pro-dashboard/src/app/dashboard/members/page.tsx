      ...
      <div className="mx-auto max-w-[1500px] space-y-6">
        ...
        {tab === "members" && (
          <section className="space-y-4">
            ...
            {!members.length && !loading ? <Empty title="Belum ada Member" text="Member yang bergabung akan muncul di sini." /> : (
              <div className="grid gap-4 xl:grid-cols-2">
                {members.map(m => <MemberCard key={m.id} member={m} send={send} />)}
              </div>
            )}
          </section>
        )}
...
function Leaderboard({ data, send, reload }: any) { return <section className="space-y-5"><form onSubmit={async e=>{e.preventDefault();const f=new FormData(e.currentTarget);const prizes=[1,2,3].map(rank=>({rank,prizeName:String(f.get(`p${rank}`)||"")})).filter(p=>p.prizeName);await send("/api/admin/members/leaderboard","POST",{name:f.get("name"),monthStart:f.get("monthStart"),monthEnd:f.get("monthEnd"),prizes});await reload();}} className={`${panel} grid gap-3 p-5 md:grid-cols-3`}><div className="md:col-span-3"><h2 className="text-xl font-black">Buat campaign leaderboard</h2><p className="text-sm text-[var(--text-muted)]">Hadiah leaderboard tidak memotong poin Member.</p></div><input name="name" required placeholder="Nama campaign" className={input}/><input name="monthStart" type="date" required className={input}/><input name="monthEnd" type="date" required className={input}/>{[1,2,3].map(i=><input key={i} name={`p${i}`} placeholder={`Hadiah Top ${i}`} className={input}/>)}<button className={`${primary} md:col-span-3`}><Medal size={16}/> Buat campaign</button></form><div className="grid gap-5 xl:grid-cols-2"><CardGrid rows={data?.campaigns} fields={["name","month_start","month_end","status"]}/><CardGrid rows={data?.prizes} fields={["rank","prize_name","notes"]}/></div></section>; }
...
function Status({status}:{status:string}) { const tone=status==="approved"||status==="completed"?"bg-emerald-400/10 text-emerald-300":status==="rejected"?"bg-rose-400/10 text-rose-300":"bg-amber-400/10 text-amber-300"; return <span className={`rounded-full px-3 py-1 text-xs font-black uppercase ${tone}`}>{status}</span>; }
