...
export async function GET(req: Request) {
  ...
  const rows = await prisma.$queryRawUnsafe<any[]>(
    `SELECT m.id,m.name,m.email,m.whatsapp,m.referral_code,m.status,m.joined_at,
     ...
     WHERE ($1='' OR m.name ILIKE '%'||$1||'%' OR m.email ILIKE '%'||$1||'%')
     GROUP BY m.id ORDER BY m.joined_at DESC LIMIT 100`, q
  );
...
export async function PATCH(req: Request) {
  ...
  if (!memberId || !status) return NextResponse.json({ error: "Data tidak lengkap" }, { status: 400 });
  if (status === "left") {
    await prisma.$executeRawUnsafe(`SELECT exit_member_program($1::uuid,$2::uuid,$3)`, memberId, a.user.id, reason || "Member keluar; seluruh poin hangus");
  } else {
  ...
  await prisma.$executeRawUnsafe(
    ...
    a.user.id, memberId, reason || null
  );
