...
export async function POST(req: Request) {
  try {
    ...
    if (!productId || !name || !email || !whatsapp) return NextResponse.json({ error: "Mohon lengkapi semua data" }, { status: 400 });

    const product = await prisma.product.findUnique({ where: { id: productId } });
    if (!product) return NextResponse.json({ error: "Produk tidak ditemukan" }, { status: 404 });

    // Member referral is parallel to Sales Creator. The browser cookie is last-click and valid for 30 days.
    let memberId: string | null = null;
    let memberReferralCode: string | null = null;
    if (affiliateCode) {
      const rows = await prisma.$queryRawUnsafe<Array<{ id: string; referral_code: string }>>(
      ...
      if (rows[0]) {
        memberId = rows[0].id;
        memberReferralCode = rows[0].referral_code;
      }
      // Invalid/expired referral never blocks normal checkout.
    }

    // Sales Creator logic intentionally unchanged.
    let resolvedSalesCode = salesCode;
    if (!resolvedSalesCode) {
      const cookiesHeader = req.headers.get("cookie") || "";
      const match = cookiesHeader.match(/sales_code=([^;]+)/);
      if (match) resolvedSalesCode = decodeURIComponent(match[1]);
    }
    let salesId = null;
    if (resolvedSalesCode) {
      const sales = await prisma.salesTeam.findFirst({ where: { code: resolvedSalesCode, status: "active" } });
      if (sales) salesId = sales.id;
    }

    let user = await findExistingCustomer(prisma, email, whatsapp);
    if (user) {
      user = await prisma.user.update({ where: { id: user.id }, data: { name, whatsapp } });
    } else {
      user = await prisma.user.create({ data: { email: String(email).trim().toLowerCase(), name, whatsapp, customerType: "new" } });
    }
    ...
    if (voucherCode) {
      ...
      if (voucher && voucher.isActive) {
        ...
        if (!isExpired && !isFull && isMinPurchaseMet) {
          discount = voucher.type === "PERCENTAGE" ? (finalAmount * Number(voucher.value)) / 100 : Number(voucher.value);
          finalAmount = Math.max(0, finalAmount - discount);
          appliedVoucherCode = code;
          await prisma.voucher.update({ where: { id: voucher.id }, data: { currentUsage: { increment: 1 } } });
        }
      }
    }

    const transaction = await prisma.transaction.create({
      data: { userId: user.id, amount: finalAmount, productName: product.name, status: "pending", source: "website", voucherCode: appliedVoucherCode, salesId }
    });
    ...
    if (!klikQrisApiKey || !klikQrisMerchantId) console.warn("KLIKQRIS_API_KEY atau KLIKQRIS_MERCHANT_ID belum diatur di .env");

    try {
      const qrisResponse = await fetch("https://klikqris.com/api/qris/create", {
        method: "POST", headers: { "Content-Type": "application/json", "x-api-key": klikQrisApiKey || "", "id_merchant": klikQrisMerchantId || "" },
        body: JSON.stringify({ order_id: transaction.id, amount: Math.round(finalAmount), id_merchant: klikQrisMerchantId, keterangan: `Pembayaran ${product.name} - ${email}` })
      });
      const qrisData = await qrisResponse.json();
      if (qrisData?.status === true && qrisData.data) {
        try { await prisma.transaction.update({ where: { id: transaction.id }, data: { paymentData: qrisData.data as any } }); }
        catch (dbUpdateErr: any) { console.error("Gagal update paymentData di DB:", dbUpdateErr.message); }
      }
      return NextResponse.json({ success: true, transactionId: transaction.id, productName: product.name, price: finalAmount, paymentData: qrisData?.status === true ? qrisData.data : null });
    } catch (error: any) {
