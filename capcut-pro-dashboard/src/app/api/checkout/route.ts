...
export async function POST(req: Request) {
  try {
    const { productId, name, email, whatsapp, memberReferralCode: requestedMemberReferralCode, voucherCode, salesCode } = await req.json();
    ...
    let resolvedMemberCode = requestedMemberReferralCode;
    ...
    // ===== Sales Creator: flow existing tetap terpisah dari Member =====
    ...
    if (user) {
      ...
      user = await prisma.user.create({
        data: { email: String(email).trim().toLowerCase(), name, whatsapp, customerType: "new" }
      });
    }
    ...
    try {
      const qrisResponse = await fetch("https://klikqris.com/api/qris/create", {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-api-key": klikQrisApiKey || "", "id_merchant": klikQrisMerchantId || "" },
